...
# twin-layer-brain のアーキテクチャ本質

時間が経っても変わらない設計思想を記録する。具体的な実装は `compiler/` `server/`
に、運用手順は `README.md` / `GUIDE.md` に書かれているが、「**なぜそうなのか**」
の根本は本ファイルに集約する。
...
- **書き込みは Markdown 側のみ**: LLM / 人間が Markdown を書く。SQL への直接
  書き込み API は公開しない。
- **再索引は自動**: Markdown が変わったら incremental indexing で SQL が追従。
...
|-------|-----------|---------------|
| `ingest` | Layer 2 (raw 読取 → wiki 書込) | 書込後に自動再索引 |
