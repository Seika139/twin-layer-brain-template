...
`--all` / `-a` を付けると、追加で `uv run ruff check compiler server tests mise/tasks/lib` と `shfmt -d mise/tasks/*.sh mise/tasks/lib/*.sh` を実行します。
...
`--all` / `-a` を付けると、追加で `uv run ruff format compiler server tests mise/tasks/lib` と `shfmt -w mise/tasks/*.sh mise/tasks/lib/*.sh` を実行します。
