...
この挙動は `mise run check-keys`、`uv run kc ...`、`mise run serve` のすべてに関係します。
...
| コマンド / 経路                                     | `.env` 扱い     | 備考                                                               |
| --------------------------------------------------- | --------------- | ------------------------------------------------------------------ |
| `uv run kc ...`                                     | あり            | `compiler.cli.main()` が `load_dotenv()` を呼ぶ                    |
| `mise run check-keys`                               | あり            | 実体は `uv run kc check-keys`                                      |
...
- `uv run kc index` は embedding を作らず、FTS5 index だけ作る
- `uv run kc search` は keyword search のみ
- `uv run kc suggest-related` は semantic search 不可として終了
