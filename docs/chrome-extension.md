...
popup を開くたびに `GET /api/auth/check` を自動で呼び、タイトル直下のバッジに状態を表示します。
Settings の「接続と token を確認」も同じ endpoint を呼び、明示的に確認したい場合に使えます。
`/api/auth/check` は `BRAIN_API_TOKEN` だけを確認する軽量 endpoint で、index や note の状態には依存しません。

バッジの色と意味:

- 緑 (接続 OK / token 有効): server に接続でき、token が一致しています。
- 黄 (Bearer Token 未設定 / API Endpoint 未設定): Settings での初期入力が必要です。
- 赤 (401 / 403 / 503 / 接続失敗): token 不一致、server 側 token 未設定、server 停止のいずれか。
