...
`kc search` (CLI), `GET /api/notes/search`, `search_notes` (MCP) から使えます。API key は不要です。
固有名詞、コマンド名、ファイル名、エラー文、設定値など「文字としてそこに書いてあるもの」を探す時に向いています。

### Semantic search

query 文と Markdown 本文の embedding 距離で近いページを探す検索です。
`kc search` (CLI), `GET /api/notes/similar`, `search_similar_notes` (MCP) から使えます。
...
`kc suggest-related` (CLI), `GET /api/notes/{id}/suggest-related`, `suggest_related_notes` (MCP) から使えます。
`OPENAI_API_KEY` が必要です。
...
`kc search` / `kc show` / `kc suggest-related` は主に LLM agent や server が内部で呼ぶ subcommand ですが、人間がデバッグや確認のために直接叩くこともできます。

人間向けのエントリポイントは 2 種類あります:

```bash
# 方法 1: mise の kc task 経由 (subcommand を -- の後ろに書く)
mise run kc -- search "Docker"
mise run kc -- show <id-or-path>
mise run kc -- suggest-related <note-id>
...
引数を付けずに `mise run kc` だけ実行すると、TTY 環境では fzf で subcommand を選択できます。

`kc search` は FTS 結果を出し、embedding が有効なら semantic search も併せて出します。

API key の状態確認は専用 wrapper を使います:
...
`--json` のようなフラグは mise 自身に食われるため、`--` を挟んで渡します。
...
複雑さも増やすので、検索結果が古くならないよう `mise run index` を運用に入れます。
